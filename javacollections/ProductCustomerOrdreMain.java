...
public class ProductCustomerOrdreMain {
	...
	//	⦁	Obtain a list of products belongs to category “Books” with price > 100
		...
		for(Product show:productlist) {
			if(show.getCategory()=="Books" && show.getPrice() > 100) {
		...
		for(Product show:productlist) {
			if(show.getCategory()=="Toys") {
				double discount=show.getPrice()/10;
				
				double priceshow = show.getPrice()- discount;
				
				System.out.println(show.getCategory() +" and Discounted price is"+priceshow);
			}
		}
		
	System.out.println("**********5**********");

Double y= 100000000000000000000000000000000000000000000000.00;
	for(Product show:productlist) {
		if(show.getCategory()=="Books") {
			Double x=show.getPrice();
